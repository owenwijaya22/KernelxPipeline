...
{
	// printf("fs_resolve called with path: %s\n", path);
	struct fs_dirent *result = 0;
	// If the path begins with a slash, navigate from the root directory.
	if(path[0] == '/') {
		// printf("Navigating from root for path: %s\n", path);
		result = fs_dirent_traverse(fs_getroot(current), &path[1]);
		// printf("fs_resolve exiting, path: %s, success: %s\n", path, (result != 0) ? "YES" : "NO");
		return result;
	}
	...
	if (colon) {
		// printf("Handling tag for path: %s\n", path);
		...
		if (!*rest) {
			result = fs_dirent_addref(tagobj->data.dir);
			// printf("fs_resolve exiting, path: %s, success: %s\n", path, (result != 0) ? "YES" : "NO");
			return result;
		}

		// Otherwise, navigate from that object.
		result = fs_dirent_traverse(tagobj->data.dir,path);
		printf("fs_resolve exiting, path: %s, success: %s\n", path, (result != 0) ? "YES" : "NO");
		return result;
	}

	// If there was no tag, then navigate from the current working directory.
	// printf("Navigating from current working directory for path: %s\n", path);
	result = fs_dirent_traverse(fs_getcurrent(current), path);
	// printf("fs_resolve exiting, path: %s, success: %s\n", path, (result != 0) ? "YES" : "NO");
	return result;
}
...
{
	// printf("fs_dirent_lookup called for name: %s\n", name);
	const struct fs_ops *ops = d->volume->fs->ops;

	if (!ops->lookup) {
		int r = 0;
		// printf("fs_dirent_lookup result for %s: %s\n", name, (r != 0) ? "FOUND" : "NOT FOUND");
		return r;
	}
...
{
	// printf("Starting fs_dirent_traverse with path: %s\n", path);

	if(!parent || !path) // check if either parent or path is NULL
		return 0;
	...
	while (part) {
		// printf("Looking up part: %s in directory\n", part);
	...
	// printf("Exiting fs_dirent_traverse, path: %s, success: %s\n", path, (d != 0) ? "YES" : "NO");
	return d;
}
...
struct fs_dirent * fs_dirent_mkdir(struct fs_dirent *d, const char *name)
{
	const struct fs_ops *ops = d->volume->fs->ops;
	if(!ops->mkdir) {
		// printf("Failed to create a filesystem directory\n");
		return 0;
	} 
	struct fs_dirent *n = ops->mkdir(d, name);
	if (n) {
		n->volume = fs_volume_addref(d->volume);
		// printf("Successfully created a filesystem directory\n");
		return n;
	}
	// printf("Failed to create a filesytem directory\n");
	return 0;
}
...
struct fs_dirent *fs_dirent_mkfile(struct fs_dirent *d, const char *name)
{
	...
	// Log to check if the mkfile operation is available
	if(!ops->mkfile) { 
		return 0;
	}
