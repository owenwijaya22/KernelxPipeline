...
int sys_make_named_pipe(const char *fname) {
	...
	// call the kernel function to create the named pipe
	int result = named_pipe_create(fname);
	if (result != 0) {
        return result; // if error occurs
    }
...
int sys_open_named_pipe(const char *fname) {
	...
	if (result < 0) {
        return KERROR_NOT_FOUND; // if named_pipe not found
    }
...
int32_t syscall_handler(syscall_t n, uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t e)
{
	...
	case SYSCALL_MAKE_NAMED_PIPE: // case for syscall make named pipe
        return sys_make_named_pipe((const char *) a);
	case SYSCALL_OPEN_NAMED_PIPE: // case for open named pipe
